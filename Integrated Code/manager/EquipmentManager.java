...
import state.EquipmentAvailableState;
import state.EquipmentInUseState;
import state.EquipmentFaultyState;
...
public class EquipmentManager {
    ...
    private EquipmentManager() {
        ...
        equipmentList.add(new Equipment("EQ006", "Hospital Bed", new EquipmentFaultyState()));
    ...
    public boolean assignEquipment(String equipmentID, Resident resident) {
        for (Equipment e : equipmentList) {
            if (e.getEquipmentID().equalsIgnoreCase(equipmentID)
                    && e.getState() instanceof EquipmentAvailableState) {
    ...
    public boolean releaseEquipment(String equipmentID) {
        for (Equipment e : equipmentList) {
            if (e.getEquipmentID().equalsIgnoreCase(equipmentID)
                    && e.getState() instanceof EquipmentInUseState) {
    ...
    public boolean reportFaultyEquipment(String equipmentID) {
        for (Equipment e : equipmentList) {
            if (e.getEquipmentID().equalsIgnoreCase(equipmentID)) {
                e.setState(new EquipmentFaultyState());
                return true;
            }
        }
        return false;
    ...
    public void displayEquipmentStatus() {
        System.out.printf("%-10s %-20s %-20s%n", "Equip ID", "Type", "Status");
        System.out.println("-----------------------------------------------------------");
        for (Equipment e : equipmentList) {
            System.out.printf("%-10s %-20s %-20s%n",
                    e.getEquipmentID(), e.getEquipmentType(), e.getState().getStatus());
        }
    }
    ...
    public List<Equipment> getAvailableByType(String type) {
        ...
        for (Equipment e : equipmentList) {
            if (e.getEquipmentType().equalsIgnoreCase(type)
                    && e.getState() instanceof EquipmentAvailableState) {
                available.add(e);
            }
        }
